...
def _run_backfill(args: argparse.Namespace) -> int:
    ...
    with psycopg.connect(config.postgres.dsn) as conn, ExitStack() as stack:
        ...
        routes = _build_routes(datasets, config, args, stack, repo)
...
def _build_routes(
    ...
    Market datasets route to SMARD (region = ``--region``); weather and telemetry route to the
    site id. Telemetry is generated by the synthetic client, which reads the day's ingested
    irradiance back through ``repo``. Every connector satisfies the same protocol, so the
    ``ingest_partition`` loop is connector-agnostic.
    """
...
def _require_weather_dependency(
    ...
    missing = [
        day
        for day in days
        if repo.latest_hash(WEATHER_SOURCE, probe, site_id, Resolution.HOUR.value, day) is None
    ]
    if missing:
        raise SystemExit(
            ...
            f"Run with --datasets weather,telemetry to seed both in dependency order, or backfill "
            f"weather for this range first."
        )
